...
module Puppet::CloudPack
  ...
  class << self
    ...
    def add_install_options(action)
      ...
      action.option '--sign-csr' do
        summary 'Sign the certificate signing request'
        description <<-EOT
          Puppet can optionally make an attempt to sign the certificate signing
          request generated by the Puppet Agent installed on the target system.
          For this to work the Puppet Master needs to be configured to allow
          your personal certificate to access the certificate signing API.  For
          security reasons, this is not the default configuration of the Puppet
          Master.

          The usual mode of operation is to run "puppet node install" as a
          regular user (not root).  This means that puppet will read it's configuration
          file from ~/.puppet/puppet.conf.  The certificate signing API requires
          a trusted client certificate so the first step is to generate one for
          yourself.

          First, make sure you have ~/.puppet/puppet.conf configured to talk to
          your Puppet CA server.  This is usually the same system as your Puppet master.

              $ cat ~/.puppet/puppet.conf
              [user]
                ca_server = puppetmaster.puppetlabs.lan

          Next, we can issue a certificate for ourselves:

              $ who am i
              jeff     pts/4        2011-10-06 15:54 (172.16.214.1)i
              $ puppet certificate generate jeff@puppetlabs.com --ca-location remote

          Then, on the Puppet CA server, sign the certificate:

              # puppet cert sign jeff@puppetlabs.com

          And retrieve the signed certificate:

              $ puppet certificate find jeff@puppetlabs.com --ca-location remote
    ...
    def install(server, options)
      ...
      # Figure out if we need to be root.  Note the trailing space on sudo is required and not a typo!
      ...
      # The PATH hackery is to accomidate Solaris which does not have /usr/local/bin in the path by default.
      universal_path="/opt/puppet/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin"
      certname_command = "#{cmd_prefix}bash -c 'PATH=#{universal_path} puppet agent --configprint certname'"
      ...
      attempt_to_sign_certificate(puppetagent_certname)

      # FIXME: Nigel says, we need to say _where_ to run this command.
      Puppet.notice "Installation of Puppet on node #{server} has finished."
      Puppet.notice "The certificate request now needs to be signed using puppet cert sign #{puppetagent_certname}"
    ...
    def attempt_to_sign_certificate(certname)
      config_file = File.join(Puppet[:confdir], 'puppet.conf')
      Puppet.notice "Attempting to sign certificate request #{certname} ..."
      Puppet.info "If you do not wish to attempt certificate signing please use the --sign-csr=false option"
      Puppet.info "Connecting to the Puppet CA API at https://#{Puppet[:ca_server]}:#{Puppet[:ca_port]}/ ..."
      Puppet.debug "The current run mode (--mode) is #{Puppet.run_mode.name.to_s}"
      Puppet.debug "The CA API Endpoint may be configured in the file #{config_file} using the ca_server and ca_port settings"
      # These variables are used by the return hash
      status = nil
      error_message = nil
      exception = nil
      certificate = nil
      begin
        # JJM: Replace this with the face itself.
        certificate_face = Puppet::Face[:certificate, :current]
        certificate = certificate_face.sign(certname, :ca_location => :remote)
        status = 'success'
      rescue => detail
        Puppet.warning "Could not sign certificate.  Please try again with --debug if you'd like more information"
        Puppet.warning "The certificate signing request can be signed by logging into the Puppet Master and executing the following command:"
        Puppet.notice  "puppet cert sign #{certname}"
        Puppet.debug "Certificate signing failed with error: #{detail} (#{detail.class})"
        status = 'failure'
